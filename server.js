...
app.get('/info', (req, res) => {
  ...
  exec(`yt-dlp --dump-json --no-playlist "${url}"`, { timeout: 30000 }, (err, stdout) => {
    ...
    try {
      ...
      const isImage = data.ext === 'jpg' || data.ext === 'jpeg' || data.ext === 'png' ||
                      data.ext === 'webp' || (!data.duration && data.url);
      res.json({
        title: data.title || 'Instagram Post',
        duration: data.duration ? formatDuration(data.duration) : 'Photo',
        channel: data.uploader || data.channel || 'Instagram',
        thumbnail: data.thumbnail,
        isImage: isImage,
        imageUrl: isImage ? (data.url || data.thumbnail) : null
      });
    } catch (e) {
...
app.get('/download', (req, res) => {
    ...
    if (!err) {
      try {
        ...
        if (isImage && data.url) {
          ...
          const dlName = `instagram_photo_${safeId}.${ext}`;
          res.setHeader('Content-Disposition', `attachment; filename="${dlName}"`);
          res.setHeader('Content-Type', 'image/jpeg');
          https.get(data.url, (imgRes) => {
            imgRes.pipe(res);
          }).on('error', () => { downloadVideo(); });
          return;
        }
      } catch (e) {}
    }
  ...
  function downloadVideo() {
    ...
    yt.on('close', (code) => {
      if (code !== 0) return res.status(500).json({ error: 'Download failed: ' + errOutput.slice(0, 200) });
